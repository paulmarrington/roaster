...
function (source_ext, compiler) {
  require.extensions[source_ext] = function(module, source) {
    morph(source, '.js',
    function(error, filename, code, save) {
...
// Helper for coffee-script derivatives to compile to
// Javascript. Has to be asynchronous because some compilers
// are asynchronous
module.exports.compileJavascript =
  function(source, compiler, next) {
    morph(source, '.js', 
      function(error, filename, content, save) {
        if (error) return next(error, filename);
        if (content) {
          try {
            var js = compiler.compile(content,
              {filename:filename});
            save(null, js);
          } catch(err) {
