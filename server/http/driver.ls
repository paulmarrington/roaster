...
module.exports = driver = (exchange) ->
  ...
  isapp = pathname.indexOf('app.coffee') != -1
    ...
    if not found # nothing cached - require until we get it or fail all paths
      ...
      for module-name in module-names
        try
          ...
          exchange.drivers.push driver-module = require module-name
          found = module-name
  ...
  drive = ->
    ...
    while exchange.drivers.length isnt 0
      driver = exchange.drivers.shift()
      exchange.last-driver = not exchange.drivers.length
      # async - run and only continue on completion
      return driver(exchange, drive) if driver.length >= 2
      # sync - do it straight away and go on to next one
      driver(exchange)
    # only get here if we have passed through all drivers
