...
export NODE_PATH=uSDLC_base_path/server:uSDLC_base_path/common:$uSDLC_node_path/server:$uSDLC_node_path/common:$uSDLC_node_path/ext/node_modules:$uSDLC_node_path/ext/node/lib/node_modules:.:$NODE_PATH
...
if [ ! -d "$uSDLC_node_path/ext/node" ]; then
    ...
    npm --silent install coffee-script send cookies node-watch node-inspector
fi
