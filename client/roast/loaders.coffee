...
require.data = (url, next, headers = pragma: "no-cache") ->
  contents = []
  @stream url, headers, (error, text, is_complete, last_modified) ->
    contents.push text
    contents = [] if error
    next(error, contents.join(''), last_modified) if is_complete
      
require.head = (url, read) ->
  url += '.script' if url.indexOf('.') is -1
  request = new XMLHttpRequest()
  request.onload = ->
    last_modified = @getResponseHeader("Last-Modified")
    read Date.parse(last_modified)
...
require.stream = (url, headers..., onData) ->
  ...
  request.onreadystatechange = ->
    ...
    onData(error, text, is_complete, last_modified)
